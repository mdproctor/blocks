...
| Path | Contents |
|------|----------|
| `summarisation-api/src/main/java/io/casehub/blocks/summarisation/` | Unified summarisation SPI — `Summariser<IN,OUT>`, `StatefulSummariser<IN,OUT,S>` (framework-managed state per partition), `ContentSummariser<T,R>` (with `asSummariser()` bridge to `StatefulSummariser`), `LevelEvent<E>` (with `@Nullable String tenancyId`), `EventStreamBus<E>`, `EventAccumulator<E>` (with `peekBuffer()`), `WindowPolicy`, `EmissionPolicy<IN,S>` (state-aware emission gating with `anyOf`/`allOf` composition), `StateStore<S>` (opt-in persistent state — `load`/`store` with write-through cache in runners), `OutputProcessor<OUT,S>` (post-summarisation output processing), `SummarisationRunner<IN,OUT>` (state-aware, builder API with `emissionPolicy()`/`stateStore()`/`stateKeyResolver()`/`outputProcessor()`), `KeyedAccumulator<K,E>` (key extractor: `Function<LevelEvent<E>, K>`), `KeyedSummarisationRunner<K,IN,OUT>` (per-key `StatefulSummariser` state + `evictState(K)`, builder API with `stateStore()`/`outputProcessor()`), `Compactor<E>`, `VerbatimContentSummariser<T>`, `Tickable` (common `tick()`/`flush()` interface for both runner types) |
...
| `summarisation-yaml/src/main/java/io/casehub/blocks/summarisation/yaml/` | YAML pipeline surface — `PipelineDefinition`, `PipelineCompiler` (windowed + keyed grouping via `ExpressionEngine`), `CompiledPipeline<IN>` (polymorphic `Tickable` runners), `SummariserRegistry`, `PipelineValidator`, `ExpressionEngine`-backed expression evaluation, Jackson YAML deserialization |
...
| `src/main/java/io/casehub/blocks/agentic/social/prompt/` | Social cognition speech integration — `SocialPromptAssembler` (compositor wrapping base `SpeechPromptAssembler` with fault-tolerant `PromptSection` composition), `SocialAvatarCognition` (`@ApplicationScoped` `AvatarCognition` CDI bean — wires all orchestrators, constructs PromptSections, handles signal recording + proactive evaluation), `ProactiveSpeechSupport` (InnerLifeOrchestrator integration for proactive initiation), 8 `PromptSection` implementations: `PersonalityPromptSection`, `MoodPromptSection` (PAD axes), `DrivePromptSection` (via `CognitiveObservationSections`), `NarrativePromptSection`, `GoalPromptSection` (`DriveGoalProposal` rendering), `MentalModelPromptSection` (BDI Theory of Mind, subject-scoped), `UserModelPromptSection` (per-subject profiles, subject-scoped), `StrategyPromptSection` |
| `src/test/java/io/casehub/blocks/agentic/social/prompt/` | Tests for social cognition speech integration |
| `src/main/java/io/casehub/blocks/agentic/social/goal/` | Autonomous goal proposal — `GoalProposalOrchestrator` (compositor), `DriveGoalMapper` SPI, four mapper implementations (Curiosity, Competence, Affiliation, Autonomy), `DriveGoalProposal` (with `@Nullable suggestedPriority` + `proposalAttributes`), `GoalProposalTick` (`NoChange`/`Changes` with proposals, abandonments, priorityAdjustments, governanceUpdates), `GoalProposalConfig`, `DriveGoalFormationStrategy` SPI with `LlmDriveGoalFormationStrategy` (drive-specific LLM prompt), `DriveGoalFormationContext`, `GoalEscalationPolicy` SPI (narrative-driven priority escalation), `NarrativeGoalEscalationPolicy` (strongest-theme alignment with count-based cap, sign-aware), `GoalEscalationConfig` (escalation/demotion cycle thresholds, cross-axis weights), `EscalationResult`/`GoalEscalationContext`, `PriorityAdjustment`/`GovernanceAttributeUpdate` (governance signals), `CrossAxisGoalEnricher` SPI with `LlmCrossAxisGoalEnricher` (LLM-enriched compound goals) |
| `src/test/java/io/casehub/blocks/agentic/social/goal/` | Tests for goal proposal |
| `src/main/java/io/casehub/blocks/agentic/social/narrative/` | Narrative identity — `NarrativeFragment` sealed hierarchy (`IndividualEpisode`, `GroupEpisode`, `DerivedTheme`), `NarrativeState` (scoped: INDIVIDUAL/GROUP), `NarrativeStore` SPI with `CbrNarrativeStore` `@DefaultBean` (CBR-backed via `NarrativeStateSchema` — polymorphic fragment serialization), `NarrativeOrchestrator` (compositor — reads NarrativeStore, caches NarrativeState per agent, `tick()` + `currentNarrative()`, per-agent ReentrantLock), `NarrativeContentSummariser` (`@ApplicationScoped` `ContentSummariser<ReflectionEntry, NarrativeState>` — LLM synthesis via AgentProvider, incremental episode merge + full theme re-derivation, degenerate response guard), `NarrativeEmissionPolicy` (`EmissionPolicy<ReflectionEntry, NarrativeState>` — count + novelty + quiet period gating via `NarrativeSynthesisGate`), `NarrativeOutputProcessor` (`OutputProcessor<NarrativeState, NarrativeState>` — episode/theme pruning by capacity and salience floor), `CbrStateStore` (`StateStore<NarrativeState>` adapter wrapping `CbrNarrativeStore`), `ReflectionEventAdapter` (pull-to-push bridge — queries `ReflectionQueryStore`, publishes `LevelEvent<ReflectionEntry>` with watermark tracking), `NarrativePipeline` (`@ApplicationScoped` factory — wires adapter → `EventStreamBus` → `SummarisationRunner` with emission policy, state store, output processor, content summariser), `NarrativeModulation` (NarrativeState → drive modulation coefficients with per-axis clamping), `NarrativeConfig`/`NarrativeSynthesisGate` (composite trigger: count + novelty + quiet period), `NarrativeTick` (Updated with `@Nullable` previous) sealed outcomes, `NarrativeScope` enum, `GroupNarrativeOrchestrator` (compositor — GROUP-scoped mirror of NarrativeOrchestrator, reads NarrativeStore with groupId, detects new GroupEpisodes/themes, exposes `members()` for consumer-provided group membership) |
| `src/test/java/io/casehub/blocks/agentic/social/narrative/` | Tests for narrative identity |
| `src/main/java/io/casehub/blocks/agentic/social/emergence/` | Social emergence — `SocialNorm` (detected behavioral pattern with `NormStrength`), `NormObservation` (interaction pattern for CBR storage), `DetectedNorms` (with `established()` filter), `NormDetectionConfig`/`NormDetectionTick`, `SocialNormDetector` (`@ApplicationScoped` compositor — reads NormObservation cases from CBR, groups by behavioralPattern, computes adherence rates, classifies strength EMERGING/ESTABLISHED/DECLINING, per-tenant caching), `NormObservationSchema` (CBR serialization), `DriveAlignment` (pairwise drive profile alignment), `CollectiveGoalProposal`/`CollectiveGoalTick`/`CollectiveGoalConfig`, `CollectiveGoalFormation` (compositor — reads N agents' DriveProfiles, computes pairwise alignment, forms groups via connected components, proposes collective goals when composite alignment exceeds threshold, per-tenant locking + per-group cooldown) |
| `src/test/java/io/casehub/blocks/agentic/social/emergence/` | Tests for social emergence |
...
| `src/main/java/io/casehub/blocks/memory/` | Memory hygiene — `MemoryHygieneOrchestrator` (tick: importance scoring → eviction → consolidation), `MemoryHygieneScheduler` (maintain: tick + reflection + peer-linking + integrity), `ConfidenceScorer` SPI with `ArousalScorer`/`SurpriseScorer`/`CompositeConfidenceScorer`, `RetentionScore`/`RetentionConfig` (composite eviction scoring), `DefaultIntegrityChecker` (structural + semantic escalation), `ReflectionEntry`/`ReflectionStore` (reflection persistence SPI), `ReflectionQueryStore` SPI with `NoOpReflectionQueryStore` `@DefaultBean` (reflection read path for `ReflectionEventAdapter`), `HygieneTick`/`MaintenanceTick` sealed outcomes, `HygieneEvent` observability |
...
| `src/main/java/io/casehub/blocks/summarisation/narrative/` | Decision narrative pipeline — `DecisionSignal` sealed hierarchy (5 signal types), `StepDecisionSummary`/`SignalDigest` (L1→L2 intermediate), `DecisionNarrative` (output), `DecisionSignalSummariser` (L1 heuristic), `DecisionNarrativeSummariser` (L2 LLM-backed `ContentSummariser` with template fallback), `NarrativeSignalStrategy` SPI, `AbstractNarrativeSignalStrategy` (base class), `DecisionNarrativePipeline` (factory wiring L1→L2) |
| `src/test/java/io/casehub/blocks/summarisation/narrative/` | Tests for decision narrative pipeline |
| `src/test/java/io/casehub/blocks/summarisation/examples/decision/` | Fsitrading-inspired decision narrative pipeline example (two-level: `TradingSignalSummariser` L1 + `TradeNarrativeSummariser` L2) |
| `speech-api/src/main/java/io/casehub/blocks/speech/` | Speech pipeline SPIs — `SpeechPromptAssembler` (`@FunctionalInterface` SPI: prompt assembly for speech sessions), `AssembledPrompt` (system + user prompt record with optional model override), `ConversationTurn` (role + content record), `PromptContext` (agentId, tenantId, subjectId per-turn context), `PromptSection` (`@FunctionalInterface` SPI: contributes nullable text sections to prompts), `AvatarCognition` (composition root SPI: wraps assembler, initialize, tick, evaluateProactive, recordInteraction), `PhonemeAligner` (`@FunctionalInterface` SPI), `LipSyncEnricher` (decorator for phoneme timing enrichment) |
| `speech-api/src/test/java/io/casehub/blocks/speech/` | Tests for LipSyncEnricher |
| `speech-sherpa/src/main/java/io/casehub/blocks/speech/sherpa/` | sherpa-onnx FFM/Panama implementation — `SherpaOnnxSpeechToText` (+ `withDefaults()` zero-install factory), `SherpaOnnxTextToSpeech` (+ `withDefaults()` zero-install factory), `VitsTextToSpeech` (direct onnxruntime + espeak-ng for TTS with exact phoneme timing), `Audio8TextToSpeech` (DualAR autoregressive TTS via ORT with streaming + voice cloning, `withDefaults()` zero-install factories, `synthesiseStreaming()` with windowed overlap-subtract codec decoding, `registerVoice()`/`releaseVoice()` for zero-shot cloning), `Audio8Tokenizer` (GPT-2 byte-level BPE tokenizer from HuggingFace `tokenizer.json`), `Audio8Config` (model variant configuration — threads, temperature, top-k), `DualARLoop` (autoregressive slow AR + fast AR inference with KV-cache and Mamba SSM state, `iterateFrames()` for streaming), `CodecDecoder` (codec token matrix → PCM waveform via ORT), `VoiceRegistry` (voice registration/release lifecycle with transient codec encoder), `RuntimeManifest` (model metadata + streaming params from JSON — `codecHopLength`, `streamContextFrames`, `streamGuardFrames`), `EspeakPhonemeAligner` (`PhonemeAligner` impl — espeak-ng phonemization with proportional duration mapping, `withDefaults()` zero-config factory), `SherpaLibrary` (native binding via FFM SymbolLookup, 3-tier loading: system path → local cache → auto-download), `OnnxRuntimeLibrary` (onnxruntime C API FFM vtable bindings, singleton env shared across sessions), `EspeakLibrary` (espeak-ng C API FFM bindings), `SherpaLayouts` (C struct layouts for sherpa-onnx 1.10.x), `Provisioner` (auto-download native lib + model with SHA-256 verification, `FileLock` concurrency, `tar xf` extraction; `ensureGectorModel()` for GECToR ONNX bundles; `ensureAudio8Model()` for Audio8 DualAR models from HuggingFace), `ModelPatcher` (ONNX model duration output patching via protobuf), `VitsConfig` (Piper model config parsing + phoneme tokenization), `SentencePieceTokenizer` (pure Java Viterbi unigram segmentation — reads .model protobuf via CodedInputStream, trie-based vocab lookup), `GectorFilter` (`TextFilter` impl, destructiveness 3 — composes WordTokenizer + SentencePieceTokenizer + ORT + GectorTagApplier for iterative grammar correction), `GectorConfig` (tag vocabulary + verb dictionary + model path resolution from model directory), `GectorTagApplier` (stateless tag application — $KEEP/$DELETE/$APPEND/$REPLACE/morphological transforms), `WordTokenizer` (spaCy-compatible word pre-tokenization with contraction splitting), `WavReader`/`WavWriter` (PCM audio I/O), `SherpaConfig`, `SherpaException` |
...
| `DriveGoalFormationStrategy` | `@FunctionalInterface` SPI: `@Nullable DriveGoalProposal propose(DriveGoalFormationContext)`. LLM-based alternative to heuristic mappers. |
| `DriveGoalFormationContext` | Record: agentId, tenantId, axis, intensity, trigger, existingGoals, remainingCapacity. |
| `LlmDriveGoalFormationStrategy` | `DriveGoalFormationStrategy` implementation via `AgentProvider`. Drive-specific prompt framing (motivational drive analyst). Graceful degradation on LLM failure. |
...
| `EventAccumulator<E>` | Thread-safe event buffer. `collect()`, `shouldEmit(now)`, `drain()`, `drainIfReady(now)` (atomic check+drain), `peekBuffer()` (read-only snapshot for `EmissionPolicy` evaluation), `clear()`, `size()`. Synchronized on all public methods. |
...
| `EmissionPolicy<IN, S>` | `@FunctionalInterface` SPI: `boolean shouldEmit(List<LevelEvent<IN>>, @Nullable S, long)`. State-aware emission gating. Static `anyOf()`/`allOf()` composition factories. |
| `StateStore<S>` | Opt-in persistent state SPI: `load(String partitionKey)` → `@Nullable S`, `store(String partitionKey, S)`. Write-through cache in runners — reads check `ConcurrentHashMap` first, fall back to store. |
| `OutputProcessor<OUT, S>` | `@FunctionalInterface` SPI: `List<OUT> process(List<OUT>, @Nullable S)`. Post-summarisation output processing — called after `summarise()`, before output bus publish. |
| `SummarisationRunner<IN, OUT>` | Wires accumulator → optional compactor → summariser → output bus. Detects `StatefulSummariser` and manages state per partition automatically. `collect()`, `tick()` (synchronized, returns `CompletionStage<Void>`), `clear()`, `size()`. Propagates tenancyId from input batch to output events. Builder API: `.emissionPolicy()` (replaces `WindowPolicy` gating), `.stateStore()` (write-through persistent state), `.stateKeyResolver()` (custom partition key from batch), `.outputProcessor()` (post-summarise processing), `.compactor()`, `.onFailure()`. Legacy `WindowPolicy` constructors unchanged. |
...
| `KeyedSummarisationRunner<K, IN, OUT>` | Wires `KeyedAccumulator` → optional compactor → `Summariser` → output bus. Same key extractor signature as `KeyedAccumulator`. Per-group failure recovery. Detects `StatefulSummariser` via `instanceof` — manages per-key state in `ConcurrentHashMap<K, Object>`. `evictState(K)` for explicit state cleanup. Implements `Tickable`. Builder API: `.stateStore()` (write-through with `K.toString()` partition key), `.outputProcessor()`, `.compactor()`, `.onFailure()`. |
...
## Sub-package: `io.casehub.blocks.summarisation.narrative`

Decision narrative pipeline — transforms platform decision signals into human-readable explanations via a two-level summarisation pipeline. Architecturally separate from identity narratives (`NarrativeFragment`/`NarrativeOrchestrator` in social cognition). Shares only the underlying summarisation primitives (`KeyedSummarisationRunner`, `EventStreamBus`, `ContentSummariser`).

| Class | What it does |
|-------|-------------|
| `DecisionSignal` | Sealed interface (5 permits): `caseId()`, `stepName()`, `timestamp()`. Pipeline input type. |
| `RoutingDecision` | Record: `selectedAgentId`, `strategyId`, `score`, `candidateIds`, `@Nullable reason` |
| `CbrRetrieval` | Record: `retrievedCount`, `topSimilarity`, `@Nullable topCaseOutcome`, `@Nullable domain` |
| `TrustAssessment` | Record: `agentId`, `trustScore`, `threshold`, `passed` |
| `DeliberationOutcome` | Record: `outcome`, `rounds`, `@Nullable convergenceState`, `participantIds` |
| `StepOutcome` | Record: `status`, `@Nullable workerId`, `@Nullable errorMessage`, `elapsed` |
| `SignalDigest` | Record: `signalType`, `summary`, `keyFacts` (Map), `confidence` — L1→L2 intermediate |
| `StepDecisionSummary` | Record: `caseId`, `stepName`, `signals` (List of SignalDigest), `from`, `to` — L1 output |
| `DecisionNarrative` | Record: `caseId`, `stepNames`, `explanation`, `evidenceSources`, `confidence`, `producedAt` — L2 output |
| `DecisionSignalSummariser` | L1 heuristic `Summariser.SyncSummariser<DecisionSignal, StepDecisionSummary>` — exhaustive sealed switch, no LLM |
| `DecisionNarrativeSummariser` | `@ApplicationScoped` L2 `ContentSummariser<StepDecisionSummary, DecisionNarrative>` — LLM-backed via `AgentProvider` with template fallback |
| `NarrativeSignalStrategy` | SPI: `onStepOutcome(Object event)`. Domain repos implement to convert platform events to `DecisionSignal`. |
| `AbstractNarrativeSignalStrategy` | Base class: `emit(DecisionSignal)` publishes to `EventStreamBus`, `onCaseClose(String)` evicts L2 state |
| `DecisionNarrativePipeline` | `@ApplicationScoped` factory: wires L1 (`KeyedSummarisationRunner` by caseId:stepName) → L2 (`KeyedSummarisationRunner` by caseId with stateful `ContentSummariser`). `tick(long)`, `evictCaseState(String)`. |
...
| `SherpaOnnxTextToSpeech` | `TextToSpeechService` implementation — VITS/Piper-based synthesis via FFM downcalls. Generates audio from text, encodes to WAV, returns `SynthesisResult`. Auto-detects TTS model filename via `findTtsModel()` directory scan. `withDefaults()`/`withDefaults(modelName)` zero-install factories, `ensureProvisioned(modelName)` for pre-download. No phoneme timing (returns empty list). |
| `KokoroTextToSpeech` | `TextToSpeechService` implementation — Kokoro (StyleTTS2) synthesis via sherpa-onnx FFM downcalls. 24kHz output, 28 voices. `withDefaults()`/`withDefaults(voiceId)` zero-install factories. No phoneme timing (returns empty list). |
| `KokoroConfig` | Record: `modelDir`, `voiceId`, `lengthScale`, `numThreads`, `provider`. `defaults(Path)` and `defaults(Path, voiceId)` factories. |
...
| `SherpaLayouts` | `StructLayout` definitions for sherpa-onnx v1.13.6 C API structs — `OFFLINE_RECOGNIZER_CONFIG`, `TTS_CONFIG` (VITS + Kokoro offsets), `GENERATED_AUDIO`. VarHandles for field access. |
...
| `OnnxRuntimeLibrary` | FFM bindings for onnxruntime C API via vtable pattern (`OrtGetApiBase`). Loads `libonnxruntime` independently (system path → sherpa-onnx cache). Singleton with shared ORT env across sessions. `Session` inner class wraps ORT session with typed inference (`runFloat`, `runRaw`), tensor creation, output count/name queries. |
...
| `SentencePieceTokenizer` | Pure Java SentencePiece tokenizer — Viterbi unigram segmentation. Reads `.model` protobuf via `CodedInputStream` (no proto compilation). Trie-based vocabulary lookup, space normalization (▁), consecutive unknown collapsing. `encode(String) → int[]`, `decode(int[]) → String`. |
| `GectorFilter` | `TextFilter` implementation (destructiveness 3, name "grammar"). Composes `WordTokenizer` + `SentencePieceTokenizer` + `OnnxRuntimeLibrary` + `GectorTagApplier` for iterative GECToR grammar correction. Per-word subword encoding, DeBERTa inference, softmax + threshold filtering, first-subword-wins tag aggregation. `AutoCloseable` — owns ORT session. |
| `GectorConfig` | Record: `modelPath`, `spModelPath`, `maxIterations` (default 5), `keepConfidence`, `minErrorProb`, `numThreads`, `provider`, `tagVocabulary`, `keepTagId`, `verbDictionary`. Factory `fromModelDir(Path)` reads `labels.txt` and optional `verb-form-vocab.txt`. |
| `GectorTagApplier` | Stateless utility: `apply(List<String> tokens, int[] tagIds, GectorConfig) → Result(tokens, changed)`. Handles $KEEP, $DELETE, $APPEND_{word}, $REPLACE_{word}, $TRANSFORM_VERB/CASE/AGREEMENT, $MERGE_HYPHEN. Verb transforms fall back to original when not in dictionary. |
| `WordTokenizer` | Rule-based word pre-tokenization matching spaCy English rules. Contraction splitting (n't, 's, 're, 've, 'll, 'd, 'm), punctuation separation. `splitSentences(String)` for sentence boundary detection. |
| `Audio8TextToSpeech` | `TextToSpeechService` implementation — DualAR autoregressive TTS via `OnnxRuntimeLibrary`. 0.1B (INT8) and 0.6B (INT4) model variants. `withDefaults()`/`withDefaults(variant)` zero-install factories, `ensureProvisioned(variant)` for pre-download. `synthesiseStreaming()` with windowed overlap-subtract codec decoding for progressive audio delivery. Voice cloning via `registerVoice(Path)`/`releaseVoice(String)` with transient codec encoder. `AutoCloseable` — owns ONNX session lifecycle. |
| `Audio8Tokenizer` | GPT-2 byte-level BPE tokenizer — parses HuggingFace `tokenizer.json`, byte-to-unicode mapping, greedy longest-match BPE merge. `encode(String) → int[]`, `decode(int[]) → String`. |
| `Audio8Config` | Record: `modelDir`, `variant`, `numThreads`, `provider`, `maxTokens`, `temperature`, `topP`, `topK`. `defaults(Path)` and `defaults(Path, variant)` factories. |
| `DualARLoop` | Autoregressive inference engine — slow AR (semantic token prediction with KV-cache + Mamba SSM state) → fast AR (codec codebook prediction per frame). `generate()` returns all frames, `iterateFrames()` yields frames via `Consumer<int[]>` callback for streaming. `AutoCloseable` — owns slow/fast AR ONNX sessions. |
| `CodecDecoder` | Codec tokens → PCM waveform via ORT. Reshapes frame matrix to `[1, codebooks, frames]` tensor, runs codec decoder session. `AutoCloseable`. |
| `VoiceRegistry` | Voice cloning lifecycle — `register(Path)` transiently loads codec encoder, encodes reference audio to codes, caches under opaque ID. `getVoiceCodes(String)`, `release(String)`, `registeredVoices()`. Thread-safe via `ConcurrentHashMap`. |
| `RuntimeManifest` | Model metadata record — parsed from `runtime_manifest.json`. Architecture params (layers, heads, dims, Mamba state), codec params (codebooks, codebook size, sample rate), streaming params (`codecHopLength`, `streamContextFrames`, `streamGuardFrames`), reference voice (`referenceCodesFile`, `referenceText`). |
| `EspeakPhonemeAligner` | `PhonemeAligner` implementation — espeak-ng phonemization via `Phonemizer` SPI + proportional duration mapping to audio length. `withDefaults()` zero-config factory (provisions espeak-ng automatically), `withEspeak(EspeakLibrary)` for explicit control. |
