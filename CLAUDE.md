...
| `src/main/java/io/casehub/blocks/agentic/personality/` | Autonomous agent personality — `PersonalityEvolutionOrchestrator` (bounded trait drift via JPAF), `InnerLifeOrchestrator` (background thought loop with proactive initiation), `TraitPressureSource<E>` + `CivilityConstraint` SPIs, `EvolutionTick`/`InnerLifeTick` sealed outcomes, default pressure sources and civility constraints |
