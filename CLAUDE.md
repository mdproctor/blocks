...
| `src/main/java/io/casehub/blocks/agentic/social/` | Agent social cognition — `PersonalityEvolutionOrchestrator` (bounded trait drift via JPAF), `InnerLifeOrchestrator` (background thought loop with proactive initiation), `UserModelOrchestrator` (per-subject profile synthesis with tiered heuristic+LLM), `TraitPressureSource<E>` + `CivilityConstraint` + `InteractionSignal` SPIs, `UserProfileStore` SPI with `CbrUserProfileStore` `@DefaultBean`, `EvolutionTick`/`InnerLifeTick`/`UserModelTick` sealed outcomes, `UserProfile`/`RelationshipStageConfig`/`StageTier`/`UserModelConfig` types, default pressure sources and civility constraints |
